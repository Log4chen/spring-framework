...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			try {
				// 空实现，3个xxxWebApplicationContext重写了该方法，其中添加了一些对几个xxxAware的ignore，及添加了一些register
				// Allows post-processing of the bean factory in context subclasses.
				postProcessBeanFactory(beanFactory);

				// invoke各BeanFactoryPostProcessor（包括其子接口BeanDefinitionRegistryPostProcessor）
				// 如果用户没有自定义PostProcessor，则只是invoke了ConfigurationClassPostProcessor
	...
	protected void prepareRefresh() {
		...
		// 空实现，只在几个xxxWebApplicationContext中进行了重写
		// Initialize any placeholder property sources in the context environment.
		initPropertySources();

		// 检查被required的properties是否存在，不存在则抛异常
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		// 添加一个BeanPostProcessor---ApplicationContextAwareProcessor，用来callback各xxxAware的setxxx方法
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		// DefaultConversionService中添加了各种Converter，用于转换不同的对象
		// 比如，controller入参中，将字符串"3,4,5"字段转换为Collection<Object>，是通过StringToCollectionConverter完成的（猜测）
		...
		// 冻结所有的bean definition，不再允许修改(包括post processor)
